export const ANNOUNCEMENTS_POLL_INTERVAL = 10000;
export const QUEUE_STATUS_POLL_INTERVAL = 30000;
export const STAFF_QUESTION_POLL_INTERVAL = 30000;
export const STUDENT_QUESTION_POS_POLL_INTERVAL = 10000;
export const STUDENT_QUOTA_POLL_INTERVAL = 60000;
export const CALENDAR_POLL_INTERVAL = 300000;
export const COURSE_TITLE_CHAR_LIMIT = 100;
export const TEXT_CHAR_LIMIT = 1000;
export const STUD_DESC_CHAR_LIMIT = 100;
export const PIN_CHAR_LIMIT = 50;
export const ALLOWED_LINKS = [
    "zoom.us",
    "zoom.com",
    "zoom.com.cn",
    "bluejeans.com",
    "meet.google.com",
    "whereby.com",
];

export const ANALYTICS_SURVEY_SHOWN_LS_TOKEN =
    "__instructor_analytics_survey_shown";
export const SPRING_2021_TRANSITION_MESSAGE_TOKEN =
    "__spring_2021_transition_message_shown";
export const ANALYTICS_SURVEY_SHOWN_SPRING_2021_END_TOKEN =
    "__analytics_survey_sp_2021_end_shown";
export const SPRING_2023_TRANSITION_MESSAGE_TOKEN =
    "__spring_2023_transition_message_shown";
export const SUMMER_2023_TRANSITION_MESSAGE_TOKEN =
    "__summer_2023_transition_message_shown";
export const FALL_2023_TRANSITION_MESSAGE_TOKEN =
    "__fall_2023_transition_message_shown";
export const CHANGELOG_TOKEN = "__changelog";

export const MOBILE_BP = "766px";
