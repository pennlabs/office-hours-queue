...
export const EditEventModal = (props: EditEventProps) => {
    ...
    const [interval, setInterval] = useState(
        !!occurrence.interval ? occurrence.interval : 15
    );
    ...
    useEffect(() => {
        ...
        setIsBookable(!!occurrence.interval);
        setInterval(!!occurrence.interval ? occurrence.interval : 15);
    ...
    const handleEditOccurrence = async () => {
        ...
        const editedOccurrence = {
            ...
            interval: isBookable ? interval : undefined,
        };

        try {
            await updateOccurrence(occurrence.id, editedOccurrence);
            setModalState(null);
        } catch (error) {
    ...
    const handleEditEvent = async () => {
        ...
        try {
            ...
            if (isBookable) {
                // Update all occurrences in the list that belong to this event
                const updatePromises = occurrences
                    .filter((occ) => occ.event.id === editedEvent.id)
                    .map((occ) =>
                        updateOccurrence(occ.id, {
                            interval: editedEvent.interval,
                        })
                    );
...
export const NewEventModal = (props: NewEventProps) => {
    ...
    const [erpDate, setErpDate] = useState(
        moment(start).add(1, "month").toDate()
    );
    ...
    useEffect(() => {
        ...
        if (lastSubmittedErp && lastSubmittedErp < end) {
            ...
            setErpDate(moment(start).add(1, "month").toDate());
        } else {
            setErpDate(
                lastSubmittedErp ?? moment(start).add(1, "month").toDate()
            );
        }
    }, [show]);
...
export const EditEventConfirmationModal = (
    props: EditEventConfirmationModalProps
) => {
    const {
        editOccurrence,
        editEvent,
