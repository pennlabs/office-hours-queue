...
import {Course, Kind, Membership, MembershipInvite, mutateFunction, Question, Queue, Semester, User,} from '../../types';
...
export const useMembers = (courseId: number, initialData: Membership[]) =>
    useResourceList(
        ...
        (id) => `/courses/${courseId}/members/${id}/`, initialData);

export const useInvitedMembers =
    (courseId: number, initialData: MembershipInvite[]) => useResourceList(
        `/courses/${courseId}/invites/`,
        (id) => `/courses/${courseId}/invites/${id}/`, initialData);
...
export function useLeadership(courseId: number, initialData: Membership[]):
  ...
  const leadership: Membership[] =
      (data || []).filter((mem) => isLeadershipRole(mem.kind));
  return [leadership, error, isValidating, mutate];
}

export async function sendMassInvites(
    courseId: number, emails: string, kind: string) {
...
export async function getSemesters(): Promise<Semester[]> {
  return doApiRequest('/semesters/').then((res) => res.json()).catch((_) => []);
}

export const useQueues = (courseId: number) => useResourceList<Queue>(
    `/courses/${courseId}/queues/`,
    (id) => `/courses/${courseId}/queues/${id}/`);

export const useQuestions =
    (courseId: number, queueId: number, refreshInterval: number) =>
...
export async function createQuestion(
    courseId: number, queueId: number,
    payload: Partial<Question>): Promise<void> {
  const res =
      await doApiRequest(`/courses/${courseId}/queues/${queueId}/questions/`, {
        method: 'POST',
        body: payload,
      });
