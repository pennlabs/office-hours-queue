...
from ohq.models import (Course, Queue, Semester)
...
class TestQuestionTemplatesMigration(MigrationTest):
    ...
    def setUpBeforeMigration(self, apps):
        self.semester = self.old_apps.get_model(self.app, 'Semester').objects.create(year=2020, term=Semester.TERM_SUMMER)
        ...
        self.queue = self.old_apps.get_model(self.app, 'Queue').objects.create(name="Queue", course=self.course)
