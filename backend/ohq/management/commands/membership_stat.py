        ...
        for membership in memberships:
            if membership.kind == Membership.KIND_STUDENT:
                questions_asked = Question.objects.filter(
                    asked_by=membership.user, status=Question.STATUS_ANSWERED, queue__course=membership.course
                )
                if questions_asked:
                    calculate_student_time_waiting(membership.user, questions_asked, membership.course) 
                    calculate_student_time_helped(membership.user, questions_asked, membership.course) 
            else:
                ...
                if questions_answered:
                    calculate_instructor_time_helping(membership.user, questions_answered, membership.course) 
                    calculate_instructor_students_per_hour(membership.user, questions_answered, membership.course)
