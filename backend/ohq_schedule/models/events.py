...
from ohq_schedule.models.calendars import Calendar
from ohq_schedule.models.rules import Rule
from ohq_schedule.utils import OccurrenceReplacer
...
class Event(models.Model):
    ...
    def clean(self):
        super().clean()
        if self.interval is not None and not self.bookable:
            raise ValidationError({'interval': _('Interval can only be set when bookable is True')})
        if self.bookable and self.interval is None:
    ...
    def _create_occurrence(self, start, end=None):
        ...
        return Occurrence(
            event=self, start=start, end=end, original_start=start, original_end=end, interval=self.interval, bookable=self.bookable
        )
...
class EventRelation(models.Model):
    ...
    content_type = models.ForeignKey(ContentType, on_delete=models.CASCADE, related_name="ohq_eventrelation_set")
...
class Occurrence(models.Model):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.interval and event:
            self.interval = event.interval
        if not self.bookable and event:
            self.bookable = event.bookable
    ...
    def clean(self):
        ...
        if self.interval is not None and not self.bookable:
            raise ValidationError({'interval': _('Interval can only be set when bookable is True')})
        if self.bookable and self.interval is None:
            raise ValidationError({'bookable': _('Bookable occurrences must have an interval set')})
